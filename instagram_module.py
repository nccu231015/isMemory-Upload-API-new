...
async def process_instagram_reel(url: str, workdir: str = "./shorts_cache") -> Dict:
    ...
    if not api_key:
        ...
        # 發送API請求
    try:
        # 標準化 Instagram URL：ScrapeCreators API 可能只接受 /reel/ 而非 /reels/
        normalized_url = url.replace("/reels/", "/reel/")
        print(f"正在請求ScrapeCreators API: {normalized_url}")

        # 手動對 URL 進行編碼，避免雙重編碼
        encoded_url = quote(normalized_url, safe="")
        full_url = f"{api_url}?url={encoded_url}"
        print(f"完整請求 URL: {full_url}")
        ...
        if "data" in data and "xdt_shortcode_media" in data["data"]:
            ...
            # 嘗試獲取描述/標題
            if (
                "edge_media_to_caption" in media_data
                and "edges" in media_data["edge_media_to_caption"]
            ):
        ...
        if not video_url:
            if (
                "media" in data
                and isinstance(data["media"], list)
                and len(data["media"]) > 0
            ):
        ...
        return {"raw_output": output, "ai_input": ai_input}
